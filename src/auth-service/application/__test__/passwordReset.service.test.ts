...
describe("ResetPasswordService", () => {
  ...
  it("debería lanzar un error si el token es inválido", async () => {
    ...
    await expect(
      service.resetPassword("invalid-token", validNewPassword)
  ...
  it("debería lanzar un error si el usuario no existe para el email en el token", async () => {
    ...
    await expect(
      service.resetPassword("valid-token", validNewPassword)
  ...
  it("debería lanzar un error si la nueva contraseña no cumple con los requisitos", async () => {
    ...
    await expect(
      service.resetPassword("valid-token", invalidNewPassword)
    ).rejects.toThrow(/Password must be 8-64 characters long/);
  });
