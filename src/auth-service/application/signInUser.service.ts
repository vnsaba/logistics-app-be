...
export class signInService {
    ...
    constructor(userRepository: IUserRepository, sms: SmsSender, passwordService: PasswordService){
    ...
    async singIn (email: string, password: string): Promise<void>{
        ...
        const isPasswordValid = await this.passwordService.comparePassword(password, user.current_password);
        if (!isPasswordValid){
