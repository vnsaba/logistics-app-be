...
export class StoreController extends Controller {
  ...
  constructor() {
    ...
    this.uploadStoreService = new UploadStoreService(storeRepository, multerFileReader);
  }

  @Post('upload')
  public async loadStores(@UploadedFile() file: Express.Multer.File) {
    return await this.uploadStoreService.upload<Express.Multer.File, { id: string; last_name: string; phone: string; isbn: string }>({
      file,
      separator: ';',
    });
  }
  ...
  @Put('{id}')
  public async updateStore(@Path() id: number, @Body() body: UpdateStoreDto): Promise<Store> {
